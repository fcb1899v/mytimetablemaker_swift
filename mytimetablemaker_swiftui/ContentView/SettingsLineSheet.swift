...
struct SettingsLineSheet: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack(alignment: .topLeading) {
                ...
                if focused && !vm.lineSuggestions.isEmpty {
                    lineSuggestionsView
                }
    ...
    private var lineSuggestionsView: some View {
        ...
        .frame(maxHeight: min(CGFloat(vm.lineSuggestions.count) * 50, 500))
        .background(RoundedRectangle(cornerRadius: 10).fill(.background))
        .overlay(RoundedRectangle(cornerRadius: 10).stroke(Color(.separator), lineWidth: 0.5))
    ...
    private var dataManagementSection: some View {
        HStack(spacing: 12) {
            ...
            if let updated = vm.lastUpdatedDisplay {
                Text("Last Updated".localized + updated)
                    .font(.caption)
